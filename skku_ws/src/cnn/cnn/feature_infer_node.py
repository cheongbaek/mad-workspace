#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Variant B 주행: 카메라 이미지 → 허프 차선특징 + 상태 → MLP → /drive_cmd.

특징 추출(lane_features.extract_lane_features)은 학습 때와 동일한 모듈을 사용한다.
입력: /image_raw, /steer_angle    출력: /drive_cmd
...
from cnn.models import FeatureMLP, make_state, decode_outputs
from cnn.lane_features import extract_lane_features
from cnn.ros_image_codec import imgmsg_to_bgr8


class FeatureInferNode(Node):
    def __init__(self):
        super().__init__("feature_infer_node")
        ...
        self.get_logger().info(
            f"feature_infer_node 시작 | max_pwm={self.max_pwm}, rate={rate:.0f}Hz")
    ...
    def _infer_cb(self):
        ...
        feat = extract_lane_features(self._latest_frame)
        state = make_state(self._cur_angle, self._prev_cmd_angle, self._prev_cmd_pwm)
        x = torch.tensor([list(feat) + state], dtype=torch.float32).to(self.device)
